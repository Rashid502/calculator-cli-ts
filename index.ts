...
console.log(
  chalk.greenBright(figlet.textSync("Calculator", { horizontalLayout: "full" }))
);
    ...
    .then(async (answer) => {
      try {
        if (answer.choice == "History (Last 10 calculations with result)") {
          ...
          console.log(
            chalk.bold.greenBright("Application closed successfully.")
          );
          process.exit(0);
        } else {
          await inquirer
            .prompt([
              {
                ...
                type: "string",
                message:
                  "Enter Your mathmatical expression (example = 8 + 9 * 5):",
              },
            ])
            .then((answer) => {
              try {
                ...
                console.log(
                  chalk.bold.blueBright("Answer: " + JSON.stringify(result))
                );
                if (history.length > 10) {
                  history.length = 0;
                  history.push(answer.expression + " = " + result);
                  fs.writeFile(
                    "last_10_calculations.txt",
